...
positiveStocks.forEach(s=>console.log(`${s.symbol}---${s.price}---${s.change}`))

// 2. Sort in ASC and DESC Order 
// Tip: while sorting, never alter the original array : make a clone and then sort
console.log("Ascending order on basis of price")
const asc_order_stock = [...stocks].sort((s1,s2)=>s1.price-s2.price)
asc_order_stock.forEach(s=>console.log(s)) 

console.log("-------------------")

console.log("Descending order:")
const desc_order_stock = [...stocks].sort((s1,s2)=>s2.price-s1.price)
desc_order_stock.forEach(s=>console.log(s))
